...
def retrieve_context(chunks: list[dict], question: str, top_k: int = 4) -> RetrievalResult:
    # Related item: retrieval returns both text context and image paths for VLM page preview.
    ...
    if not ranked:
        return RetrievalResult(context_text="관련 문맥을 찾지 못했습니다.", image_paths=[])
    ...
    return RetrievalResult(context_text=context_text, image_paths=image_paths)
