...
class RuntimeGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            main_tab,
            text="설명: ColPali+Byaldi는 이미지 기반 RAG입니다. 각 PDF 페이지 이미지를 생성해 검색/응답에 활용합니다.",
        ...
        self.api_base_entry = LabeledEntry(chat_tab, "외부 VLM API Base URL")
        ...
        self.chat_question = tk.Text(chat_tab, height=5)
        ...
        preview_frame = ttk.LabelFrame(chat_tab, text="검색된 페이지 이미지 미리보기")
        preview_frame.pack(fill="x", pady=(4, 8))
        self.image_preview_label = ttk.Label(preview_frame, text="이미지 없음")
        self.image_preview_label.pack(fill="x", padx=6, pady=6)

        self.chat_context = tk.Text(chat_tab, height=10)
        ttk.Label(chat_tab, text="검색된 문맥 (Byaldi 검색 결과)").pack(anchor="w")
    ...
    def show_architecture_proposal(self) -> None:
        proposal = (
            ...
            "3) 생성 단계(VLM API):\n"
            "- 검색된 문맥과 이미지 근거를 기준으로 외부 VLM 응답 생성\n"
            "- VLM 탭에서 API 키 연결상태 확인 후 질의\n"
        )
    ...
    def check_vlm_connection(self) -> None:
        ...
        if not settings.api_key or not settings.api_base:
            messagebox.showwarning("확인", "VLM API Key와 Base URL을 입력하세요.")
            return
    ...
    def ask_question(self) -> None:
        ...
        if not all([settings.api_key, settings.api_base, settings.model]):
            messagebox.showwarning("확인", "VLM 탭에서 API Key/Base/모델명을 모두 입력하세요.")
            return
