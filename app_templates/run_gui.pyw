...
class RuntimeGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.ask_button = ttk.Button(chat_tab, text="질문 전송", command=self.ask_question)
        self.ask_button.pack(anchor="w", pady=(8, 8))
    ...
    def ask_question(self) -> None:
        ...
        self.status_var.set("VLM 응답 생성 중")

        def _worker() -> None:
            try:
                llm_context = retrieval.llm_context_text or retrieval.context_text
                answer = OpenAICompatibleClient(settings).chat(question, llm_context)
                self.after(0, lambda: self._on_answer_success(answer))
            except Exception as exc:  # noqa: BLE001
                self.after(0, lambda: self._on_answer_error(str(exc)))

        threading.Thread(target=_worker, daemon=True).start()

    def _on_answer_success(self, answer: str) -> None:
        self.chat_answer.delete("1.0", "end")
        self.chat_answer.insert("end", answer)
        self.ask_button.configure(state="normal")
        self.status_var.set("VLM 응답 완료")

    def _on_answer_error(self, error_text: str) -> None:
        self.ask_button.configure(state="normal")
        self.status_var.set("VLM 응답 실패")
        messagebox.showerror("VLM 호출 오류", error_text)
