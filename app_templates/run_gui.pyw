...
class RuntimeGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            main_tab,
            text="설명: 인덱스 타입은 고정(colpali_byaldi)이며, 원본 폴더의 PDF를 Byaldi 검색용 로컬 인덱스로 구성합니다.",
        ...
        ttk.Button(chat_tab, text="VLM 설정 저장", command=self.save_config).pack(anchor="w", pady=(0, 8))
    ...
    def show_architecture_proposal(self) -> None:
        proposal = (
            ...
            "1) 인덱싱 단계(로컬 무료):\n"
            "- 원본 폴더의 PDF를 페이지 단위 텍스트/메타로 분할\n"
            "- Byaldi 검색에 맞춰 문맥 조각(rag_chunks.jsonl) 생성\n"
            "- 인덱스 메타(rag_index_manifest.json)에 index_type=colpali_byaldi 기록\n\n"
            "2) 검색 단계(Byaldi):\n"
            "- 사용자 질문과 문맥 조각을 점수화해 Top-K 문맥 반환\n"
            "- 추후 실제 Byaldi 임베딩/벡터 검색으로 쉽게 교체 가능\n\n"
            "3) 생성 단계(VLM API):\n"
            "- 검색된 문맥만 외부 VLM에 전달\n"
            "- Hallucination을 줄이기 위해 system prompt에서 문맥 기반 응답 강제\n\n"
            "현재 앱은 1~3단계를 분리하여 유지보수성과 교체 용이성을 우선 설계했습니다."
        )
