...
class OpenAICompatibleClient:
    ...
    def _normalized_base_url(self) -> str:
        # Related item: keep Google AI Studio endpoint path valid without forcing '/v1'.
    ...
    def chat(self, prompt: str, context: str) -> str:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "너는 실무형 분석 도우미다. 문맥을 근거로 추론해 답하되, 문장을 그대로 복붙하지 말고 핵심을 재구성해라. 반드시 한국어로 답하고, 결과는 [결론], [근거], [계산/판단], [주의사항] 순서로 작성해라. 문맥에 없는 정보는 추정임을 명시해라.",
                },
                {
                    "role": "user",
                    "content": f"문맥:\n{context}\n\n질문:\n{prompt}",
